    ...
    def qtwist_description(self):
        if self.contains_negative_one:
            if len(self.qtwists) > 1:
                return r"yes  (see %s for level structures without $-I$)"%(', '.join([modcurve_link(label) for label in self.qtwists[1:]]))
            else:
                return r"yes"
        else:
            return r"no  (see %s for the level structure with $-I$)"%(modcurve_link(self.qtwists[0]))
